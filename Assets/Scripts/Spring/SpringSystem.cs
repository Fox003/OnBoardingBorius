...
partial struct SpringSystem : ISystem
{
    [BurstCompile]
    public void OnCreate(ref SystemState state)
    {
        state.RequireForUpdate<SpringData>();
    }
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach (var (transform, spring) in SystemAPI.Query<RefRW<LocalTransform>, RefRW<SpringData>>())
        {
            var motionParams = spring.ValueRW.motionParams;
            var currentPosition = spring.ValueRW.CurrentPos;
            var velocity = spring.ValueRW.Velocity;
            var dampingRatio = spring.ValueRW.DampingRatio;
            var frequency = spring.ValueRW.Frequency;
            var targetPos = spring.ValueRW.TargetPos;
            
            CalcDampedSpringMotionParams(ref motionParams, deltaTime, frequency, dampingRatio);
            UpdateDampedSpringMotion(ref currentPosition, ref velocity, targetPos, motionParams);
        }
    }
    ...
    //******************************************************************************
    private void CalcDampedSpringMotionParams(
    ...
    //******************************************************************************
    private void UpdateDampedSpringMotion(
